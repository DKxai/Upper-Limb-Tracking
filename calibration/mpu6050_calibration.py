...
def _read_gyro(path):
    """Đọc + BỎ các dòng rỗng/thiếu (ô stage hay gyro trống → NaN), tránh RuntimeWarning khi ép int."""
    return _read(path).dropna(subset=["stage", "gx_dps", "gy_dps", "gz_dps"]).reset_index(drop=True)


def _read_accel(path):
    """Đọc + BỎ các dòng rỗng/thiếu (ax/ay/az trống → NaN)."""
    return _read(path).dropna(subset=["ax_g", "ay_g", "az_g"]).reset_index(drop=True)
